...
/* Includes ---------------------------------------------------------------- */
#include <miguelgimenez-Inclinaciones_inferencing.h>
...
void loop()
{
    ...
    // Encontrar la clase con mayor probabilidad y encender LED
    float max_val = 0;
    int max_idx = 0;
    ...
    ei_printf(">>> Prediccion: %s\r\n", label.c_str());

    apagar_leds();
    if      (label == "Roll_pos")  set_color(true,  false, false); // Rojo
    else if (label == "Roll_neg")  set_color(true,  false, true);  // Rosa
    else if (label == "Pitch_pos") set_color(false, true,  false); // Verde
    else if (label == "Pitch_neg") set_color(true,  true,  false); // Amarillo
    else if (label == "Yaw_pos")   set_color(false, false, true);  // Azul
    else if (label == "Yaw_neg")   set_color(true,  false, true);  // Morado
